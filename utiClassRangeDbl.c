...
{ int   *nxp;                                       /** pointeur sur array des nx **/
  ...
  double xm,x,xmin,xmax;

  n=nz;  for(i=0; i<n; i++){ indp[i]=i;}
  if(n < 2 ){ return ;}

  nxp = intAlloc(nxsize, "ranindDbl");
  *nxp=n-1;  *(nxp+1)=0;  kx=1;
  ...
  { in = *(nxp+kx);  ix = *(nxp+kx-1);
    ...
    { xm=0. ;
      for(i=in; i<=ix; i++) { xm = xm + *(xp+indp[i]) ;}
      xm=xm/( (double)(ix-in+1) ) ;                          /** moyenne calculee **/
...
/**recherche d un premier element >= moyenne xm; 
   ...
   si n existe pas alors choisir xm=valeur du premier element et declarer j1ex=in!**/
      ...
      if(j1l==0)                                               /** cas malheureux **/
      { xmin = xmax = xm = *(xp+indp[in]);
        for(j1=in; j1<=ix; j1++)
        { x = *(xp+indp[j1]);
          if(x > xmax){ xmax = x;}
          else if(x < xmin){ xmin = x;}
        }
        if(xmax==xmin){ kx=kx-1;  *(nxp+kx) = *(nxp+kx)+1;  continue;}
        j1l=1; j1ex=in; jnmin=in+1;
      }        
                                                        /** donc j1l=1 toujours ! **/
      ...
      {    /** debut des 2 boucles de recherche de swap dans intervalle jmin,jmax **/
        ...
        {                                         /** debut cas j1ex n'existe pas **/
          ...
          { myErr0(-1,stderr,"ranindDbl erreur : ne doit pas arriver \n" );
          }
...
/*      fprintf(stderr," kx=%6d ,in=%6d  ,ix=%6d ,jnx=%6d \n",kx,in,ix,jnx);  */
      ...
      { kx++;
        if(kx >= nxsize)
        { nxp=intChkRealloc(nxp,&nxsize,(size_t)kx+1,10,"ranindDbl");
        }
...
/******************************************************************************/
/*        classDbl(indexp,x,xp,nx)                                            */
/* given a table of nx double (in xp) ordered in ascending order,             */
/* class a new double x.                                                      */
...
{ int       indx,indc,indw;
  ...
  myErr1(-1,stderr,"classDbl : ERROR ", 
